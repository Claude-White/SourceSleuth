...
async function fetchWebPage(url) {
  ...
  try {
    ...
    const lowValueKeywords = ['404', 'not found', 'access denied', 'forbidden', 'login'];
    if (lowValueKeywords.some(keyword => title.toLowerCase().includes(keyword))) {
      throw new Error('Low value page detected');
    }
    
    // More targeted content extraction - focus on main content areas first
    const contentSelectors = [
      'main article', 'article', 'main', '.post-content', 
      '.article-content', '#content', '.content', '.main',
      '[role="main"]', '.entry-content', '.post', '.blog-post'
    ];
    
    // Remove non-content elements site-wide before extraction
    $('script, style, nav, footer, header, aside, iframe, .comments, .sidebar, .advertisement, .ads, .navigation, [role="banner"], [role="navigation"], [role="complementary"]').remove();
    ...
    if (content.length > MAX_CONTENT_LENGTH) {
      ...
      const lastPart = content.substring(content.length - MAX_CONTENT_LENGTH * 0.3); // 30% from end
...
async function getWebContext(query) {
  try {
    ...
    const webPageTexts = webPages.map((page, index) => 
...
`;
    ...
    const sourceMatches = webContext.match(/\[Source \d+: .+?\] \((.+?)\)/g) || [];
